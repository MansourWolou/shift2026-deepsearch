"""
DeepSearch Agent — LangChain + Llama 3.2 3B
Par defaut, utilise Llama 3.2 3B via endpoint OpenAI-compatible (OpenRouter).
"""
...
DEFAULT_MODEL = os.getenv("LLAMA32_MODEL", "meta-llama/llama-3.2-3b-instruct")
...
def build_agent(model: str = DEFAULT_MODEL, temperature: float = 0.1):
    """Construit un agent DeepSearch base sur Llama 3.2 3B + Tavily."""
    llm = ChatOpenAI(
        model=model,
...
def run(query: str, model: str = DEFAULT_MODEL) -> str:
    ...
    console.print(
        Panel(
            f"[bold blue]DeepSearch Llama 3.2[/bold blue] · [dim]{model}[/dim]\n[white]{query}[/white]",
            border_style="blue",
        )
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DeepSearch Llama 3.2 3B")
    parser.add_argument("query", nargs="*", help="Question a rechercher")
    parser.add_argument(
        "--model",
        default=DEFAULT_MODEL,
        help=f"Modele Llama (defaut: {DEFAULT_MODEL})",
    )
